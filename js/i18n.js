...
(function (global) {
  ...
  var DICT = {
    en: {
      ...
      styleAuto: 'Auto — AI article-style, offline cloze fallback',
      ...
      footerNote: 'No accounts — your puzzles never leave this browser',
      ...
      aiTesting: 'Testing…',
      aiTestOk: 'AI service reachable.',

      'err.aiUnavailable': 'The AI settings are unavailable right now — please check the configuration.',
      'err.aiUnreachable': 'Could not reach the AI endpoint — check the Base URL, and note some providers block browser (CORS) requests.',
      'err.aiDailyQuota': 'Today’s AI quota is used up — please try again tomorrow.',
      'err.aiRateLimited': 'Too many AI requests — please wait about a minute and try again.',
      'err.aiUpstream': 'The AI service is temporarily unavailable — please try again later.',
      'err.aiBadRequest': 'The AI request was rejected ({msg}).',
      'err.aiBadResponse': 'AI returned an unreadable response — please try again.',
      'err.aiTimeout': 'The AI service did not respond in time — please check the configuration and try again later.',
      'err.aiHttp': 'AI request failed (HTTP {status}).',
    ...
    zh: {
      ...
      addWordPh: '补充一个单词',
      clueStyleL: '线索风格',
      styleAuto: '自动——优先 AI 文章式，失败时回落原文填空',
      styleCloze: '原文填空（离线）',
      styleAi: 'AI 文章式',
      ...
      'err.aiUnavailable': '当前 AI 设置不可用，请检查配置。',
      'err.aiUnreachable': '无法连接 AI 接口——请检查 Base URL；部分服务商禁止浏览器直连（CORS）。',
      'err.aiDailyQuota': '今日 AI 额度已用完，请明天再试。',
      'err.aiRateLimited': 'AI 请求过于频繁，请约一分钟后再试。',
      'err.aiUpstream': 'AI 服务暂时不可用，请稍后重试。',
      'err.aiBadRequest': 'AI 请求被拒绝（{msg}）。',
      'err.aiBadResponse': 'AI 返回的内容无法解析，请重试。',
      'err.aiTimeout': 'AI 服务无响应，请检查配置后稍后再试。',
      'err.aiHttp': 'AI 请求失败（HTTP {status}）。',
